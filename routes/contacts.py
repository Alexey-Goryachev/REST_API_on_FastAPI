...
async def search_contacts(first_name: str = None, last_name: str = None, email: EmailStr = None, db: Session = Depends(get_db)):
    contacts = await repository_contacts.search_contacts(first_name, last_name, email, db)
    if len(contacts) == 0:
...
async def read_birthdays_contacts(start_date: date = date.today(), 
    ...
    contacts = await repository_contacts.get_birthday_contacts(start_date, end_date, db)
    if len(contacts) == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contacts not found")
    return contacts
